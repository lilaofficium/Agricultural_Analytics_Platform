...
logger = get_logger("silver.transformer.Temperature")

def Transform_Temperature(df: pd.DataFrame, rules: dict) -> tuple[pd.DataFrame, pd.DataFrame]:  
    logger.debug(f"Incoming columns: {list(df.columns)}")
    logger.debug(f"Incoming dtypes:\n{df.dtypes}")
    df = df.drop(columns=["_id"]) 
    df = df.drop(columns=["_loaded_at"])  
    required_columns = ["year", "country", "avg_temp"] 
